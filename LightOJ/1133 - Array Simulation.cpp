...
int main()
{
    ...
    while(tc--)
    {
        ...
        rep(j,op)
        {
            ...
            int D;
            sf("%c",&ch);
            if(ch=='S'){ sf("%d",&D);getchar(); rep(i,n) v[i]+=D;}
            else if(ch=='M'){ sf("%d",&D);getchar(); rep(i,n) v[i]*=D;}
            else if(ch=='R'){  reverse(v.begin(),v.end());getchar();}
            else if(ch=='D'){ sf("%d",&D);getchar(); rep(i,n) v[i]/=D;}
            else if(ch=='P'){ int x,y; sf("%d %d",&x,&y);getchar(); swap(v[x],v[y]);}
        }
        pf("Case %d:\n",cas++);
        pf("%d",v[0]);
        repl(i,v.size()-1) pf(" %d",v[i]);
        puts("");
        v.clear();//rep(i,3) pf("%d ",v[i]);
    }
    return 0;
}
