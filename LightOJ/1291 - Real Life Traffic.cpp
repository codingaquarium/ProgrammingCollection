...
int dfscounter,ans,ncomp;
int dfs_num[MX],dfs_low[MX],dfs_parent[MX],num[MX],par[MX];
bool vis[MX];
	...
	for(i = 0; i < sz; i++) {
		...
		if(dfs_num[v]==white) {
			...
			if(dfs_low[v] > dfs_num[u]) {
			    //printf("call:");cout<<"Boss : "<<u<<" num -> "<<num[v]<<" "<<v<<endl;
				dfs2(v,ncomp++);
				//cout<<"coming out:"<<num[v]<<" "<<v<<endl;
				//puts("ends");
            }
		}
