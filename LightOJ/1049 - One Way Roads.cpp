...
int bfs(int src,int ct)
{
    queue<int> q;
    q.push(src);
    int taken[110]={0};
    taken[1]=1;    taken[src] = 1;
    int cost = ct;
    bool fg = true;
    while(!q.empty())
    {
...
int main()
{
    ...
    while(tc--)
    {
        ...
        //int cst=0;
        //cout<< edge[1].size() <<" : "<< edge[1][0].ft <<" "<< edge[1][1].ft <<endl;

        int clc = bfs(edge[1][0].ft,edge[1][0].sd);
        int anclc = bfs(edge[1][1].ft,edge[1][1].sd);
