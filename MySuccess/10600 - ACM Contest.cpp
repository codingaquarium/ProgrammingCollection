...
int krus2(G g[],int edge,int ver,int p)
{
    int sum = 0,cnt=0;
    initSet(ver+10);
    for(int i=0;i<edge;i++)
...
int main()
{
    ...
    while(t--)
    {
        ...
        sort(g,g+edge,comp);
        int m2=1<<30,m1 = krus(g,edge,node);
        if(edge==1){ printf("%d %d\n",m1,m1); continue; }
        rep(i,v.size())
        {
            int m = krus2(g,edge,node,v[i]);
            if(m!=-1)
                m2 = min(m2,m);
        }
        v.clear();
